...
class _RulerScrollAreaState<T> extends State<RulerScrollArea<T>> {
        ...
        builder: (context, constraints) {
          return ScrollConfiguration(
            ...
            child: ListView.builder(
              key: _listViewKey,
              physics: isCustomStop
                  ? AlwaysScrollableScrollPhysics()
                  : LatchingScrollPhysics(widget.indicatorExtend),
  ...
  void _scrollPositionChangeHandler() {
    ...
    // test
    if (widget.rulerController.isCustomValue) {
      setState(() {
        isCustomStop = true;
      });
    } else {
    ...
    if (index != widget.rulerController.index ||
        widget.rulerController.customValue !=
            widget.rulerController.getValueAt(index)) {
  ...
  void _valueChangeHandler() {
    if (widget.rulerController.isCustomValue) {
      ...
      if (!_userIsScrolling) {
        _jumpToIndex(widget.rulerController.customValue);
      }
  ...
  double _indexToOffset(num index) =>
  ...
  void _jumpToIndex(num index) {
    final double position = _indexToOffset(index);
    // final double position = 108;
